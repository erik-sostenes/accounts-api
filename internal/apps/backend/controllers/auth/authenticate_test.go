package auth

import (
	"context"
	"github.com/erik-sostenes/accounts-api/internal/mooc/account/business/domain"
	account2 "github.com/erik-sostenes/accounts-api/internal/mooc/account/business/services"
	"github.com/erik-sostenes/accounts-api/internal/mooc/account/business/services/find"
	"github.com/erik-sostenes/accounts-api/internal/mooc/account/infrastructure/persistence"
	"github.com/erik-sostenes/accounts-api/internal/mooc/auth/business/services"
	"github.com/erik-sostenes/accounts-api/internal/mooc/auth/business/services/authenticate"
	"github.com/erik-sostenes/accounts-api/internal/shared/backend/jwt"
	"github.com/erik-sostenes/accounts-api/internal/shared/mooc/business/domain/query"
	"net/http"
	"net/http/httptest"
	"testing"
)

const privateKey = `-----BEGIN RSA PRIVATE KEY-----

Tkqn9t2JRnRp7OXIpdXPxe+glrrYZA365rb4aZ048RTT3omcREG4
-----END RSA PRIVATE KEY-----`

const publicKey = `-----BEGIN PUBLIC KEY-----

EQIDAQAB
-----END PUBLIC KEY-----`

type AuthControllerFunc func() (AuthController, error)

var account = func() domain.Account {
	account, err := domain.NewFirstAccount().Account()
	if err != nil {
		panic(err)
	}

	return account
}()

func TestAuthController_Authenticate(t *testing.T) {
	tsc := map[string]struct {
		*http.Request
		AuthControllerFunc
		expectedStatusCode int
	}{
		"Given an authentication with a valid existing account, a status code 200 is expected with the token": {
			Request: httptest.NewRequest(http.MethodGet,
				"/v1/account/authenticate?id=94343721-6baa-4cd5-a0b4-6c5d0419c02d&password=secret", http.NoBody),
			AuthControllerFunc: func() (controller AuthController, err error) {
				mockStore := persistence.NewMockStore[domain.AccountId, domain.Account]()

				if err = mockStore.Save(context.TODO(), account.AccountId(), account); err != nil {
					return
				}

				findAccountQueryHandler := find.FindAccountQueryHandler{
					AccountFinder: find.NewAccountFinder(mockStore),
				}

				bus := make(query.QueryBus[find.FindAccountQuery, account2.AccountResponse])

				if err = bus.Record(find.FindAccountQuery{}, &findAccountQueryHandler); err != nil {
					return
				}

				authenticateAccountQueryHandler := authenticate.AuthenticateAccountQueryHandler{
					authenticate.NewAccountAuthenticator(&bus, jwt.NewJWT([]byte(privateKey), []byte(publicKey))),
				}

				queryBus := make(query.QueryBus[authenticate.AuthenticateAccountQuery, services.AuthResponse])

				if err := queryBus.Record(authenticate.AuthenticateAccountQuery{}, &authenticateAccountQueryHandler); err != nil {
					return controller, err
				}

				return NewAuthController(&queryBus), nil
			},
			expectedStatusCode: http.StatusOK,
		},
		"Given an authentication with invalid credentials with an existing valid account, a status code 401 is expected": {
			Request: httptest.NewRequest(http.MethodGet,
				"/v1/account/authenticate?id=94343721-6baa-4cd5-a0b4-6c5d0419c02d&password=12344", http.NoBody),
			AuthControllerFunc: func() (controller AuthController, err error) {
				mockStore := persistence.NewMockStore[domain.AccountId, domain.Account]()

				if err = mockStore.Save(context.TODO(), account.AccountId(), account); err != nil {
					return
				}

				findAccountQueryHandler := find.FindAccountQueryHandler{
					AccountFinder: find.NewAccountFinder(mockStore),
				}

				bus := make(query.QueryBus[find.FindAccountQuery, account2.AccountResponse])

				if err = bus.Record(find.FindAccountQuery{}, &findAccountQueryHandler); err != nil {
					return
				}

				authenticateAccountQueryHandler := authenticate.AuthenticateAccountQueryHandler{
					authenticate.NewAccountAuthenticator(&bus, jwt.NewJWT([]byte(privateKey), []byte(publicKey))),
				}

				queryBus := make(query.QueryBus[authenticate.AuthenticateAccountQuery, services.AuthResponse])

				if err := queryBus.Record(authenticate.AuthenticateAccountQuery{}, &authenticateAccountQueryHandler); err != nil {
					return controller, err
				}

				return NewAuthController(&queryBus), nil
			},
			expectedStatusCode: http.StatusUnauthorized,
		},
		"Given an authentication with a no-existing account, a status code 403 is expected": {
			Request: httptest.NewRequest(http.MethodGet,
				"/v1/account/authenticate?id=94343721-6baa-4cd5-a0b4-6c5d0419c02d&password=secret", http.NoBody),
			AuthControllerFunc: func() (controller AuthController, err error) {
				mockStore := persistence.NewMockStore[domain.AccountId, domain.Account]()

				findAccountQueryHandler := find.FindAccountQueryHandler{
					AccountFinder: find.NewAccountFinder(mockStore),
				}

				bus := make(query.QueryBus[find.FindAccountQuery, account2.AccountResponse])

				if err = bus.Record(find.FindAccountQuery{}, &findAccountQueryHandler); err != nil {
					return
				}

				authenticateAccountQueryHandler := authenticate.AuthenticateAccountQueryHandler{
					authenticate.NewAccountAuthenticator(&bus, jwt.NewJWT([]byte(privateKey), []byte(publicKey))),
				}

				queryBus := make(query.QueryBus[authenticate.AuthenticateAccountQuery, services.AuthResponse])

				if err := queryBus.Record(authenticate.AuthenticateAccountQuery{}, &authenticateAccountQueryHandler); err != nil {
					return controller, err
				}

				return NewAuthController(&queryBus), nil
			},
			expectedStatusCode: http.StatusForbidden,
		},
	}

	for name, ts := range tsc {
		t.Run(name, func(t *testing.T) {
			controller, err := ts.AuthControllerFunc()
			if err != nil {
				t.Fatal(err)
			}

			req := ts.Request
			resp := httptest.NewRecorder()

			controller.Authenticate(resp, req)

			if ts.expectedStatusCode != resp.Code {
				t.Errorf("status code was expected %d, but it was obtained %d", ts.expectedStatusCode, resp.Code)
			}
		})
	}
}
